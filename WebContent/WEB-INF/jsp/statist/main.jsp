	...
	function user_reload() {
		$.ajax({
			...
			success : function(data) {
				...
				var ymds = _(data.list).map(function(temp){
					return (temp.ymd+"时");
				});
				var pvs = _(data.list).map(function(temp){
					return Math.ceil(temp.pageview);
				});
				var uvs = _(data.list).map(function(temp){
					return temp.userview;
				});
				var chart = $('<canvas  width="800" height="300"></canvas>');
				$("#myChart").html(chart);
 ...
 */	                    ]
	...
	$(function() {
		...
		var arry = str.split("ymd=")
		if(arry&&arry[1]){
			$("input[name=ymd]").val(arry[1]);
	...
	<div class="container-fluid">
		<div class="row">
			...
			<form class="form-inline mt-2 mt-md-0" action="#"
				id="user_query_form">
				条件<input type="text" class="form-control mr-sm-2" name="ymd">
				<!--每页 -->
