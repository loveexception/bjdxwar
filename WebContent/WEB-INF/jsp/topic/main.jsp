...
<body>
	...
	<div class="container-fluid">
		<div class="row">
			...
			<!-- 	类型<input type="text" name="type" value="133"> -->
 				类型<select selected="selected" name="type" >
					<option value="0">全部</option>
					<option value="1">要闻</option>
					<option value="133">视频</option>
					<option value="6">图片</option>
					<option value="134">新机场</option>
					<option value="101">镇街</option>
					<option value="5">便民</option>
					<option value="127">女性</option>
					<option value="10">科普</option>
					<option value="3">旅游</option>
					<option value="7">文体</option>
					<option value="4">健康</option>
					<option value="9">教育</option>
					<option value="21">专题</option>
				</select> 页数<input type="text" name="pageNumber" value="1">
			...
			<script type="text/template" id="tpl">
		        {{_.each(obj,function(topic){}}
		            <tr > 
		                <td> {{=topic.oldid }} </td>
						<td> <a href="https://m2.beijingdaxing.cn/Home/news/detail/id/{{=topic.oldid}}" target="_blank">{{=topic.name }}</a> </td>
		                <td> {{=topic.readed }} </td>
		                ...
		                <td><a class="btn btn-outline-warning" href="${base}/subtopic/main?tid={{=topic.id}}" target="_blank">计划</a></td>
                		<td>
							{{ if(topic.sort>1){ }} 
							<button class="btn btn-info" onclick='user_update({{=topic.id }},0);'>
								 取消置顶 
							</button>
							{{ }else{ }}
							<button class="btn btn-outline-info" onclick='user_update({{=topic.id}},10000);'>
								 置顶 
							</button>
							{{ } }}	
						</td>
